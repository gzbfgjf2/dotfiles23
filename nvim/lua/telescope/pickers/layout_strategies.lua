...
local function validate_layout_config(strategy_name, configuration, values,
  ...
  default_layout_config = vim.F.if_nil(default_layout_config,
  ...
  return function(self, max_columns, max_lines, override_layout)
    return layout(
      ...
      validate_layout_config(
        name,
        layout_config,
        vim.tbl_deep_extend("keep", vim.F.if_nil(override_layout, {}),
...
layout_strategies.horizontal = make_documented_layout(
  ...
  function(self, max_columns, max_lines, layout_config)
    ...
    local height = resolve.resolve_height(height_opt)(self, max_columns,
      max_lines)
    ...
    elseif layout_config.prompt_position == "bottom" then
      ...
      error(string.format("Unknown prompt_position: %s\n%s",
        self.window.prompt_position, vim.inspect(layout_config)))
    end

    local anchor_pos = resolve.resolve_anchor_pos(layout_config.anchor or "",
      width, height, max_columns, max_lines)
...
layout_strategies.center = make_documented_layout(
  ...
  function(self, max_columns, max_lines, layout_config)
    ...
    results.col, preview.col, prompt.col = width_padding, width_padding,
        width_padding

    local anchor = layout_config.anchor or ""
    local anchor_pos = resolve.resolve_anchor_pos(anchor, width, height,
...
layout_strategies.bottom_pane = make_documented_layout(
  ...
  function(self, max_columns, max_lines, layout_config)
    ...
    if self.previewer and max_columns >= layout_config.preview_cutoff then
      -- Cap over/undersized width (with preview)
      local width, w_space = calc_size_and_spacing(max_columns, max_columns, bs,
        2, 4, 0)
